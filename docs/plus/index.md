...
**cdk8s+** is vended as a separate library for each kubernetes spec version. The documentation presented here represents version [1.23.0](https://github.com/kubernetes/kubernetes/tree/v1.23.0/api/openapi-spec)
and is vended as the `cdk8s-plus-23` library.
...
any spec version higher or equal to `1.23.0`. **Non-stable** resources are not guaranteed to be compatible, as they may be removed in future spec versions.

!!! warning

    If you are deploying manifests produced by `cdk8s-plus-23` onto clusters of a lower version, you might encounter some unsupported spec properties or invalid manifests.
...
deployment.expose({ serviceType: kplus.ServiceType.LOAD_BALANCER });
...
=== "TypeScript"
    ...
    `❯ npm install cdk8s-plus-23 cdk8s constructs`
...
=== "JavaScript"
    ...
    ```typescript
    const kplus = require('cdk8s-plus-23');
...
=== "Python"
    ...
    `❯ pip install --pre cdk8s-plus-23 cdk8s`
...
=== "Java"
    ...
    <dependency>
      <groupId>org.cdk8s</groupId>
      <artifactId>cdk8s-plus-23</artifactId>
...
=== "Go"
    ...
    import (
      ...
      "github.com/cdk8s-team/cdk8s-plus-go/cdk8splus23"
    )
    ```
    ...
    cdk8splus23.NewDeployment(chart, jsii.String("Deployment"), &cdk8splus23.DeploymentProps{
      Replicas: jsii.Number(3),
      Containers: &[]*cdk8splus23.ContainerProps{{
